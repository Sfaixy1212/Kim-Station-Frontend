...
function MonthInput({ label, value, onChange }) {
  return (
    <div>
      ...
      <input type="month" value={value} onChange={(e)=>onChange(e.target.value)} className="border rounded px-3 py-2 text-sm w-full" />
    </div>
  );
}
...
export default function DealerTrend() {
  ...
  return (
    <>
      ...
      <Card title="Cerca dealer" subtitle="Inserisci almeno 2 caratteri. Nessun filtro: sono inclusi tutti i dealer.">
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
          <div className="relative">
            ...
            <input
              ...
              onKeyDown={onKeyDown}
              placeholder="Cerca per Ragione Sociale, P.IVA, Codice, ecc."
            ...
            {openSuggest && query.trim().length >= 2 && !loadingSuggest && (
              <div className="absolute z-[60] mt-1 bg-white border rounded w-full max-h-64 overflow-auto shadow">
                {suggest.length === 0 ? (
                  <div className="px-3 py-2 text-sm text-gray-500">Nessun dealer trovato</div>
                ) : (
                  suggest.map((d, idx)=> (
                    <button
                      key={d.IDDealer || d.id}
                      type="button"
                      className={`w-full text-left px-3 py-2 text-sm ${idx===activeIdx ? 'bg-blue-50' : 'hover:bg-gray-50'}`}
                      onMouseEnter={()=> setActiveIdx(idx)}
                      onMouseDown={(e)=> e.preventDefault()}
                      onClick={()=>handlePick(d)}
                    >
                      <div className="font-medium">{d.RagioneSociale || d.name}</div>
                      <div className="text-xs text-gray-500">ID: {d.IDDealer || d.DealerID || d.id} {d.PartitaIVA ? `• PIVA ${d.PartitaIVA}` : ''}</div>
                    </button>
                  ))
                )}
              </div>
        ...
        <div className="mt-4 flex items-center gap-2">
          <button type="button" onClick={runTrend} disabled={loadingTrend || !dealer} className={`px-4 py-2 rounded-md text-white text-sm ${loadingTrend ? 'bg-gray-400' : 'bg-blue-600 hover:bg-blue-700'}`}>{loadingTrend ? 'Elaborazione…' : 'Vedi andamento'}</button>
          <button type="button" onClick={()=>{ setDealer(null); setQuery(''); setTrend(null); }} className="px-3 py-2 text-sm rounded-md bg-gray-100 hover:bg-gray-200 text-gray-700">Reset</button>
        </div>
      </Card>
