...
export default function CompensiAgenti() {
  ...
  const load = async () => {
    ...
    try {
      ...
      // Normalizza le chiavi della riga ai nomi attesi della vista vw_compensi_agenti_mese_agg
      ...
      const normalized = list.map(r => ({
        ...
        Euro_Totale: toNum(pick(r, ['Euro_Totale'])),
  ...
  const totals = useMemo(() => {
    ...
    return {
      // KPI dalla vista vw_compensi_agenti_mese_compensi
  ...
  // Compensi (€) - Aggiornati con nuova vista
  const compensiCards = [
    ...
    { key: 'EURO_TOTALE', title: '€ TOTALE', value: formatEuro(totals.Euro_Totale), icon: '💶', color: 'green' },
  ];
        ...
        <Card title="Filtri & Ricerca">
          <form onSubmit={(e)=>{ e.preventDefault(); load(); }}>
            <div className="flex flex-wrap items-end gap-3">
              ...
              <button type="button" disabled={!rows.length || loading} onClick={exportXlsx} className={`ml-auto px-3 py-2 text-sm rounded-md border ${(!rows.length||loading)?'text-gray-400 cursor-not-allowed':'text-gray-700 hover:bg-gray-50'}`}>📊 Esporta XLSX</button>
            </div>
          </form>
        </Card>
              ...
              <div className="p-6 overflow-auto max-h-[60vh]">
                ...
                <div className="mt-6 p-4 bg-gray-50 rounded-lg">
                  ...
                  <ul className="text-sm text-gray-700 space-y-1">
                    <li>• <strong>Fonte dati:</strong> Vista <code>vw_compensi_agenti_mese_compensi</code></li>
